...
//        boolean valid = checkParent(object, getParentEditComponent((DomainObjectEditPanel) component));
        boolean valid = checkDistrictData(object, component);
        return valid;
    }
...
//    }
    private boolean checkDistrictData(DomainObject object, Component component) {
        ...
        if ((entityTypeId == OrganizationStrategy.CALCULATION_CENTER) && !Strings.isEmpty(districtCode)) {
        ...
        if ((entityTypeId == OrganizationStrategy.CALCULATION_CENTER) && Strings.isEmpty(districtName)) {
            component.getPage().error(ResourceUtil.getString(OrganizationStrategy.RESOURCE_BUNDLE, "calc_center_must_have_district_name", component.getLocale()));
            validated = false;
        }
        return validated;
    }
