...
public class AddressService extends AbstractBean {
    ...
    private void resolveLocalAddress(ActualPayment actualPayment, long userOrganizationId) {
        ...
        if (streetTypeCorrections.size() == 1) {
            ...
            if (streetTypeIds.size() == 1) {
                ...
                streetTypeCorrection = addressCorrectionBean.createStreetTypeCorrection(streetType.toUpperCase(),
                        streetTypeId, osznId, MODULE_ID, userOrganizationId);
        ...
        Long buildingId;
        BuildingCorrection buildingCorrection;
        ...
        if (buildingCorrections.size() == 1) {
            ...
            if (buildingIds.size() == 1) {
                ...
                buildingCorrection = addressCorrectionBean.createBuildingCorrection(buildingNumber.toUpperCase(), buildingCorp.toUpperCase(),
                        streetCorrection.getId(), buildingId, osznId, MODULE_ID,
                        userOrganizationId);
    ...
    public void resolveLocalStreet(DwellingCharacteristics dwellingCharacteristics, long userOrganizationId) {
        ...
        //связывание типа улицы
        String streetType = dwellingCharacteristics.getStreetTypeReference();
        Long streetTypeId;
        Correction streetTypeCorrection;
        ...
        if (streetTypeCorrections.size() == 1) {
            ...
            dwellingCharacteristics.setStatus(RequestStatus.MORE_ONE_LOCAL_STREET_TYPE_CORRECTION);
            return;
        } else {
            List<Long> streetTypeIds = addressCorrectionBean.findInternalStreetTypeIds(streetType);
            if (streetTypeIds.size() == 1) {
                ...
                streetTypeCorrection = addressCorrectionBean.createStreetTypeCorrection(streetType.toUpperCase(),
                        streetTypeId, osznId, MODULE_ID, userOrganizationId);
                streetTypeCorrection.setExternalId(dwellingCharacteristics.getStreetTypeReferenceCode());

                addressCorrectionBean.insert(streetTypeCorrection);
            } else if (streetTypeIds.size() > 1) {
                dwellingCharacteristics.setStatus(RequestStatus.MORE_ONE_LOCAL_STREET_TYPE);
                return;
            } else {
                dwellingCharacteristics.setStatus(RequestStatus.STREET_TYPE_UNRESOLVED_LOCALLY);
                return;
            }
        }
    ...
    public void resolveLocalStreet(FacilityServiceType facilityServiceType, long userOrganizationId) {
        ...
        //связывание типа улицы
        String streetType = facilityServiceType.getStreetTypeReference();
        Long streetTypeId;
        Correction streetTypeCorrection;
        List<Correction> streetTypeCorrections = addressCorrectionBean.findStreetTypeLocalCorrections(streetType,
                osznId, userOrganizationId);
        ...
        if (streetTypeCorrections.size() == 1) {
            ...
            facilityServiceType.setStatus(RequestStatus.MORE_ONE_LOCAL_STREET_TYPE_CORRECTION);
            return;
        } else {
            List<Long> streetTypeIds = addressCorrectionBean.findInternalStreetTypeIds(streetType);
            if (streetTypeIds.size() == 1) {
                ...
                facilityServiceType.setStatus(RequestStatus.MORE_ONE_LOCAL_STREET_TYPE);
                return;
            } else {
                facilityServiceType.setStatus(RequestStatus.STREET_TYPE_UNRESOLVED_LOCALLY);
                return;
            }
        }
    ...
    private Correction findOrCreateStreetTypeCorrection(AbstractRequest request, String streetType, long streetTypeObjectId,
        ...
        if (streetTypeCorrections.size() == 1) {
            ...
            streetTypeCorrection = addressCorrectionBean.createStreetTypeCorrection(streetType.toUpperCase(),
                    streetTypeObjectId, osznId, MODULE_ID, userOrganizationId);
            addressCorrectionBean.insert(streetTypeCorrection);
            return streetTypeCorrection;
        }
    }
    ...
    public void resolveOutgoingAddress(Payment payment, CalculationContext calculationContext) {
        ...
        if (cityCorrections.size() == 1) {
            ...
            adapter.prepareCity(payment, cityCorrection.getCorrection(), cityCorrection.getExternalId());
        } else if (cityCorrections.size() > 1) {
        ...
        adapter.prepareStreet(payment, streetCorrection.getCorrection(), streetCorrection.getExternalId());
        //получаем тип улицы
        ...
        if (streetTypeCorrection == null) {
            ...
            adapter.prepareStreetType(payment, streetTypeCorrection.getCorrection(), streetTypeCorrection.getExternalId());
        }
        ...
        if (buildingCorrections.size() == 1) {
            ...
            adapter.prepareBuilding(payment, buildingCorrection.getCorrection(), buildingCorrection.getCorrectionCorp(),
                    buildingCorrection.getExternalId());
        } else if (buildingCorrections.size() > 1) {
    ...
    public void resolveOutgoingDistrict(Payment payment, CalculationContext calculationContext) {
        ...
        if (districtCorrections.size() == 1) {
            ...
            adapter.prepareDistrict(payment, districtCorrection.getCorrection(), districtCorrection.getExternalId());
    ...
    public void resolveOutgoingAddress(ActualPayment actualPayment, CalculationContext calculationContext) {
        ...
        if (cityCorrections.size() == 1) {
            Correction cityCorrection = cityCorrections.get(0);
            adapter.prepareCity(actualPayment, cityCorrection.getCorrection(), cityCorrection.getExternalId());
        } else if (cityCorrections.size() > 1) {
        ...
        adapter.prepareStreet(actualPayment, streetCorrection.getCorrection(), streetCorrection.getExternalId());
        //получаем тип улицы
        Correction streetTypeCorrection = streetCorrection.getStreetTypeCorrection();
        if (streetTypeCorrection == null) {
            ...
            adapter.prepareStreetType(actualPayment, streetTypeCorrection.getCorrection(), streetTypeCorrection.getExternalId());
        }
    ...
    public void resolveOutgoingAddress(Subsidy subsidy, CalculationContext calculationContext) {
        ...
        if (cityCorrections.size() == 1) {
            Correction cityCorrection = cityCorrections.get(0);
            adapter.prepareCity(subsidy, cityCorrection.getCorrection(), cityCorrection.getExternalId());
        } else if (cityCorrections.size() > 1) {
        ...
        adapter.prepareStreet(subsidy, streetCorrection.getCorrection(), streetCorrection.getExternalId());
        //получаем тип улицы
        Correction streetTypeCorrection = streetCorrection.getStreetTypeCorrection();
        if (streetTypeCorrection == null) {
            ...
            adapter.prepareStreetType(subsidy, streetTypeCorrection.getCorrection(), streetTypeCorrection.getExternalId());
        }
    ...
    @Transactional
    public void resolveOutgoingAddress(DwellingCharacteristics dwellingCharacteristics, CalculationContext context) {
        List<Correction> cityCorrections = addressCorrectionBean.findCityRemoteCorrections(context.getCalculationCenterId(),
                dwellingCharacteristics.getInternalCityId());
        if (cityCorrections.size() == 1) {
            Correction cityCorrection = cityCorrections.get(0);
            adapter.prepareCity(dwellingCharacteristics, cityCorrection.getCorrection(), cityCorrection.getExternalId());
        } else if (cityCorrections.size() > 1) {
        ...
        //поиск района
        resolveOutgoingDistrict(dwellingCharacteristics, context);
        ...
        //поиск улицы
        StreetCorrection streetCorrection;
        List<StreetCorrection> streetCorrections = addressCorrectionBean.findStreetRemoteCorrections(context.getCalculationCenterId(),
                dwellingCharacteristics.getInternalStreetId());
        if (streetCorrections.size() == 1) {
            streetCorrection = streetCorrections.get(0);
        } else if (streetCorrections.size() > 1) {
            streetCorrections = addressCorrectionBean.findStreetRemoteCorrectionsByBuilding(context.getCalculationCenterId(),
        ...
        adapter.prepareStreet(dwellingCharacteristics, streetCorrection.getCorrection(), streetCorrection.getExternalId());
        ...
        if (streetTypeCorrection == null) {
            ...
            adapter.prepareStreetType(dwellingCharacteristics, streetTypeCorrection.getCorrection(), streetTypeCorrection.getExternalId());
        }

        //поиск дома
        List<BuildingCorrection> buildingCorrections = addressCorrectionBean.findBuildingRemoteCorrections(context.getCalculationCenterId(),
        ...
        //квартиры не ищем, а подставляем напрямую, обрезая пробелы.
    ...
    public void resolveOutgoingAddress(FacilityServiceType facilityServiceType, CalculationContext calculationContext) {
        ...
        if (cityCorrections.size() == 1) {
            Correction cityCorrection = cityCorrections.get(0);
            adapter.prepareCity(facilityServiceType, cityCorrection.getCorrection(), cityCorrection.getExternalId());
        } else if (cityCorrections.size() > 1) {
        ...
        adapter.prepareStreet(facilityServiceType, streetCorrection.getCorrection(), streetCorrection.getExternalId());
        //получаем тип улицы
        Correction streetTypeCorrection = streetCorrection.getStreetTypeCorrection();
        if (streetTypeCorrection == null) {
            ...
            adapter.prepareStreetType(facilityServiceType, streetTypeCorrection.getCorrection(), streetTypeCorrection.getExternalId());
        }
    ...
    public void resolveOutgoingDistrict(ActualPayment actualPayment, CalculationContext calculationContext) {
        ...
        if (districtCorrections.size() == 1) {
            Correction districtCorrection = districtCorrections.get(0);
            adapter.prepareDistrict(actualPayment, districtCorrection.getCorrection(), districtCorrection.getExternalId());
        } else if (districtCorrections.size() > 1) {
    ...
    public void resolveOutgoingDistrict(Subsidy subsidy, CalculationContext calculationContext) {
        ...
        if (districtCorrections.size() == 1) {
            Correction districtCorrection = districtCorrections.get(0);
            adapter.prepareDistrict(subsidy, districtCorrection.getCorrection(), districtCorrection.getExternalId());
        } else if (districtCorrections.size() > 1) {
    ...
    public void resolveOutgoingDistrict(DwellingCharacteristics dwellingCharacteristics, CalculationContext calculationContext) {
        ...
        if (districtCorrections.size() == 1) {
            Correction districtCorrection = districtCorrections.get(0);
            adapter.prepareDistrict(dwellingCharacteristics, districtCorrection.getCorrection(), districtCorrection.getExternalId());
        } else if (districtCorrections.size() > 1) {
    ...
    public void resolveOutgoingDistrict(FacilityServiceType facilityServiceType, CalculationContext calculationContext) {
        ...
        if (districtCorrections.size() == 1) {
            Correction districtCorrection = districtCorrections.get(0);
            adapter.prepareDistrict(facilityServiceType, districtCorrection.getCorrection(), districtCorrection.getExternalId());
        } else if (districtCorrections.size() > 1) {
...
//                                }
    ...
    @Transactional
    public void correctLocalAddress(DwellingCharacteristics dc, CORRECTED_ENTITY entity, Long cityId,
                                    Long streetTypeId, Long streetId, Long buildingId, long userOrganizationId)
            throws DublicateCorrectionException, MoreOneCorrectionException, NotFoundCorrectionException {
        final long osznId = dc.getOrganizationId();
        final long requestFileId = dc.getRequestFileId();

        String city = dc.getCity();
        String streetCode = dc.getStringField(DwellingCharacteristicsDBF.CDUL);
        String buildingNumber = dc.getStringField(DwellingCharacteristicsDBF.HOUSE);
        String buildingCorp = dc.getStringField(DwellingCharacteristicsDBF.BUILD);
        ...
        switch (entity) {
            ...
            break;
            case STREET_TYPE: {
                String streetType = dc.getStreetTypeReference();
                ...
                if (streetTypeCorrections.isEmpty()){
                    ...
                    streetTypeCorrection.setExternalId(dc.getStreetTypeReferenceCode());

                    addressCorrectionBean.insert(streetTypeCorrection);

                    dwellingCharacteristicsBean.markCorrected(requestFileId, dc.getStreetTypeReferenceCode(), null);
                }else {
                    throw new DublicateCorrectionException();
                }
            }
            ...
            case STREET: {
                ...
                if (cityCorrections.size() == 1) {
                    ...
                    if (streetCorrections.size() > 0) {
                        ...
                        dwellingCharacteristicsBean.markCorrected(requestFileId, null, streetCode);
                    }
                } else if (cityCorrections.size() > 1) {
                    throw new MoreOneCorrectionException("city");
                } else {
            ...
            case BUILDING: {
                ...
                if (cityCorrections.size() == 1) {
                    ...
                    final String streetType = dc.getStreetType();
                    List<Correction> streetTypeCorrections = addressCorrectionBean.findStreetTypeLocalCorrections(streetType,
                            osznId, userOrganizationId);
                    if (streetTypeCorrections.size() == 1) {
                        Correction streetTypeCorrection = streetTypeCorrections.get(0);

                        final String street = dc.getStreet();
                        List<StreetCorrection> streetCorrections = addressCorrectionBean.findStreetLocalCorrections(cityCorrection.getId(),
                                streetTypeCorrection.getId(), street, osznId, userOrganizationId);
                        if (streetCorrections.size() == 1) {
                            StreetCorrection streetCorrection = streetCorrections.get(0);
                            ...
                            if (buildingCorrections.size() > 0) {
                                ...
                                dwellingCharacteristicsBean.markCorrected(requestFileId, null, streetCode, buildingNumber, buildingCorp);
                            }
                        } else if (streetCorrections.size() > 1) {
                            throw new MoreOneCorrectionException("street");
                        } else {
    ...
    @Transactional
    public void correctLocalAddress(FacilityServiceType f, CORRECTED_ENTITY entity, Long cityId,
                                    Long streetTypeId, Long streetId, Long buildingId, long userOrganizationId)
            throws DublicateCorrectionException, MoreOneCorrectionException, NotFoundCorrectionException {
        final long osznId = f.getOrganizationId();
        final long requestFileId = f.getRequestFileId();

        String city = f.getCity();
        String streetCode = f.getStringField(FacilityServiceTypeDBF.CDUL);
        String buildingNumber = f.getStringField(FacilityServiceTypeDBF.HOUSE);
        String buildingCorp = f.getStringField(FacilityServiceTypeDBF.BUILD);
        ...
        switch (entity) {
            case CITY: {
                ...
                if (cityCorrections.size() > 0) {
                    ...
                    Correction cityCorrection = addressCorrectionBean.createCityCorrection(city.toUpperCase(), cityId, osznId,
                            MODULE_ID, userOrganizationId);
                    addressCorrectionBean.insert(cityCorrection);
                    facilityServiceTypeBean.markCorrected(requestFileId);
                }
            }
            break;
            case STREET_TYPE: {
                String streetType = f.getStreetTypeReference();
                ...
                if (streetTypeCorrections.isEmpty()){
                    ...
                    streetTypeCorrection.setExternalId(f.getStreetTypeReferenceCode());

                    addressCorrectionBean.insert(streetTypeCorrection);

                    facilityServiceTypeBean.markCorrected(requestFileId, f.getStreetTypeReferenceCode(), null);
                }else {
                    throw new DublicateCorrectionException();
                }
            }
            ...
            case STREET: {
                ...
                if (cityCorrections.size() == 1) {
                    ...
                    if (streetCorrections.size() > 0) {
                        ...
                        facilityServiceTypeBean.markCorrected(requestFileId, null, streetCode);
                    }
                } else if (cityCorrections.size() > 1) {
                    throw new MoreOneCorrectionException("city");
                } else {
            ...
            case BUILDING: {
                ...
                if (cityCorrections.size() == 1) {
                    ...
                    final String streetType = f.getStreetType();
                    List<Correction> streetTypeCorrections = addressCorrectionBean.findStreetTypeLocalCorrections(streetType,
                            osznId, userOrganizationId);
                    if (streetTypeCorrections.size() == 1) {
                        Correction streetTypeCorrection = streetTypeCorrections.get(0);

                        final String street = f.getStreet();
                        List<StreetCorrection> streetCorrections = addressCorrectionBean.findStreetLocalCorrections(cityCorrection.getId(),
                                streetTypeCorrection.getId(), street, osznId, userOrganizationId);
                        if (streetCorrections.size() == 1) {
                            StreetCorrection streetCorrection = streetCorrections.get(0);
                            ...
                            if (buildingCorrections.size() > 0) {
                                ...
                                facilityServiceTypeBean.markCorrected(requestFileId, null, streetCode, buildingNumber, buildingCorp);
                            }
                        } else if (streetCorrections.size() > 1) {
                            throw new MoreOneCorrectionException("street");
                        } else {
